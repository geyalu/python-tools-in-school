...
try:
    ...
    def authenticate_block(block):
        APDU=[0xFF,0x86,0x00,0x00,0x05,0x01,0x00,0x3E,0x61,0x00]  #0x60==keyA   0x61=keyB
        APDU[-3]=block
        response = SCardTransmit(hcard,dwActiveProtocol,APDU)
        if response[1][0]==144:
    ...
    def read_keys():
        f=open(r'M1_Keys.txt','r')
        keys=[]
        allLines = f.readlines()
        f.close()
        for eachline in allLines:
            keys.append(eachline.strip())
